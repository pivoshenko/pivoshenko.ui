...
This repo needs Node.js 24 with [pnpm](https://pnpm.io), and `just`.
...
| --- | --- |
| `just install` | Installs the project dependencies |
| `just format` | Formats the project |
| `just lint` | Lints the project |
| `just test` | No test command yet; skipped when the `.no-tests` sentinel is present |
| `just check` | Runs `lint` then `test` in sequence |
| `just update` | Upgrades the dependencies recursively across the workspace |
...
| --- | --- | --- |
| CI | Push to `main`, pull requests, `workflow_dispatch` | Single `ci` job on `ubuntu-24.04-arm`; installs dependencies with pnpm on Node.js 24, then lints and tests the project |
| Release | `workflow_dispatch` (optional `version` input) | Two chained jobs on `ubuntu-24.04-arm`: `tag` installs dependencies with pnpm on Node.js 24 and lints the project, resolves the next version from the `version` input or from the commit history via git-cliff, bumps the `package.json` version, regenerates the changelog, then pushes `main` with the new tag; `release` publishes the GitHub Release for that tag with the generated notes as its body |
