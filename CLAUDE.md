...
- **Peer deps for framework + Tailwind.** React, Next, `@vercel/analytics`, Tailwind are peers (all optional) — sites bring their own versions; config-only consumers don't trip on missing React. Runtime `dependencies`: `lucide-react` (icons used by `inputs`/`footer`/`scroll-to-top`) — pulled transitively into sites, no consumer install needed.
